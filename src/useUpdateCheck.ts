...
// time as __APP_VERSION__) against the latest GitHub release. No auto-update (that needs a Developer
// ID + notarization) — this just surfaces a notification dot + a download link in Settings › About.
// One unauthenticated request per app session; failures (offline, rate limit) are ignored silently.
const REPO = 'yjzhng/ChemViewer';
...
export function useUpdateCheck(): UpdateInfo {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.github.com/repos/${REPO}/releases/latest`, {
      ...
      .catch(() => {
        /* offline / rate-limited → no update shown */
      });
    return () => {
      cancelled = true;
    };
